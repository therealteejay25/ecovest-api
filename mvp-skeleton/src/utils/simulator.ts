...
/**
 * Simulate a simple projection series over durationMonths.
 * expectedReturn: percent over whole duration (e.g., 12 for 12%)
 */
export const simulateProjection = (
  ...
  for (let m = 1; m <= Math.max(durationMonths, 1); m++) {
    const randomFactor = (Math.random() - 0.5) * vol; // stochastic component
